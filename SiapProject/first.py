...
def Summarize(live, firsts, others):
    ...
    d = statFunctions.CohenEffectSize(firsts.prglngth, others.prglngth)
    print('Cohen d', d)
...
def MakeHists(live):
    ...
    others: DataFrame
    """
    hist = statFunctions.Hist(live.birthwgt_lb, label='birthwgt_lb')
    thinkplot.Hist(hist)
    ...
    hist = statFunctions.Hist(live.birthwgt_oz, label='birthwgt_oz')
    thinkplot.Hist(hist)
    ...
    hist = statFunctions.Hist(np.floor(live.agepreg), label='agepreg')
    thinkplot.Hist(hist)
    thinkplot.Save(root='first_agepreg_hist', 
                   xlabel='years',
                   ylabel='frequency')

    hist = statFunctions.Hist(live.prglngth, label='prglngth')
    thinkplot.Hist(hist)
...
def MakeComparison(firsts, others):
    ...
    others: DataFrame
    """
    first_hist = statFunctions.Hist(firsts.prglngth, label='first')
    other_hist = statFunctions.Hist(others.prglngth, label='other')
